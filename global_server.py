...
#this script acts as a websocket-server, checking which client is
#connecting and sending commands on whether to stream or not.
#It also acts as two TCP servers,
#relaying the stream to appropriate websocket.



EXPECTED_CLIENTS = ("rasp_main", "rasp_sec", "oculus_client_main"\
    ,"oculus_client_sec", "tracking_client")
...
class IncomingStreamHandler(TCPServer):
    ...
    def set_clients(self, camName, wsName):
        ...
        #Attributes:
        #camName: name of incoming camera streamer i.e
        #"rasp_main" or "rasp_sec"
        #wsName: name of recieving websocket. i.e
        #"oculus_client_main" or "oculus_client_sec"
...
class SocketHandler(websocket.WebSocketHandler):
    ...
    def on_message(self, message):
        ...
        if message in EXPECTED_CLIENTS:
            ...
            or (self.name == "rasp_main" and "oculus_client_main" in cName):
                cName["rasp_main"].write_message("start_stream")
            if (self.name == "oculus_client_sec" and "rasp_sec" in cName) or\
            (self.name == "rasp_sec" and "oculus_client_sec" in cName):
